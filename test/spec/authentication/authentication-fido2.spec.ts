...
describe('Authentication - FIDO2', () => {
	...
	async function createTestPrincipal(testName: string): Promise<Principal> {
		...
		const principals = await repo.find({
			where: { login: principal.login },
		});

		return { ...principal, id: principals[0].id };
	}
	...
	async function setupTestPrincipal(
		...
		const principalId = await auth.createPrincipal({ login });
	...
	beforeAll(async () => {
		...
		// (End-to-End tests use createTestPrincipal for isolation)
		testPrincipal = {
			login: 'testuser@example.com',
		};

		await repo.insert({
			records: [testPrincipal],
		});

		// Get the principal ID
		const principals = await repo.find({
			where: { login: testPrincipal.login },
		});
		const principalId = principals[0].id;

		testPrincipal.id = principalId;
	});
	...
	describe('generateRegistrationOptions', () => {
		it('should generate valid registration options', async () => {
			...
			expect(options.user.displayName).toBe(testPrincipal.login);
	...
	describe('createPrincipal', () => {
		it('should create principal and generate options', async () => {
			const newPrincipal: Principal = {
				login: 'newuser-create-principal@example.com',
			};

			const principalId = await auth.createPrincipal(newPrincipal);
			...
			expect(createdPrincipal!.login).toBe(newPrincipal.login);
		...
		describe('getAuthenticatorByCredentialID', () => {
			it('should return authenticator credential by ID', async () => {
				...
				if (credential) {
					...
					expect(credential.principal_id).toEqual(
						helperTestPrincipal.id!
					);
